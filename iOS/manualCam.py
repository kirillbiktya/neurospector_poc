...
class CMTime(Structure):
    _fields_ = [('value', CMTimeValue), ('timescale', CMTimeScale), ('flags', CMTimeFlags), ('epoch', CMTimeEpoch)]
...
class CAVCaptureWhiteBalanceGain(Structure):
    _fields_ = [('blueGain', ctypes.c_float),  # values must be between 1.0 and maxWhiteBalanceGain
        ('greenGain', ctypes.c_float), ('redGains', ctypes.c_float)]
...
def manualCapture(orientation, exposureMode, exposureValue, exposureScale, iso,  # 23-736?
        focusMode, focusDistance, temperatureAndTint, torch, fileName, albumName):
    ...
    # delegate
    CameraManualPhotoCaptureDelegate = create_objc_class('CameraManualPhotoCaptureDelegate', methods=[
    ...
    if exposureValue and exposureScale and iso:
        device.setExposureModeCustomWithDuration_ISO_completionHandler_((CMTime(exposureValue, exposureScale, 1, 0)),
            (iso), None, restype=None, argtypes=[CMTime, c_float, c_void_p])
    ...
    if temperatureAndTint:
        ...
        device.deviceWhiteBalanceGainsForTemperatureAndTintValues_(
            (AVCaptureWhiteBalanceTemperatureAndTintValues(temperatureAndTint[1])), restype=None,
            argtypes=[AVCaptureWhiteBalanceTemperatureAndTintValues])
    # time.sleep(0.1)
